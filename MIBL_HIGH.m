...
data_full = [pname 'MIBL HIGH DR FUZZ ALL PHASES.ctf'];
data_mono = [pname 'MIBL HIGH DR FUZZ NO HEX OR HEMATITE MONO.ctf'];
data_met = [pname 'MIBL HIGH DR FUZZ NO HEX OR HEMATITE METAL.ctf'];
...
CS = cs_loader({'mono','Pt','metal','tet','suboxide','hematite'})
...
grains_mono = create_grains(ebsd_mono,'misorientation',10,'smallest_grain',1,'smoothing',1,'fill_gaps','no','phase_name','Monoclinic ZrO$$_2$$')
...
grain_dimension_hist_ellipse(grains_mono,'bin_size',5,'max_size',1000,'units','nm','max_percentage',10)
