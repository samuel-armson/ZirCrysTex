...
%data_mono_38NVa1= [pname '38NV_first_200.ctf'];
...
ebsd_mono_LOWDR = dataset_rotation(ebsd_mono_LOWDR,[0,25,0],'axis','keep','keepXY');
ebsd_mono_MEDDR = dataset_rotation(ebsd_mono_MEDDR,[0,30,0],'axis','keep','keepXY');
ebsd_mono_1TDa1 = dataset_rotation(ebsd_mono_1TDa1,[-5,5,0],'axis','keep','keepXY');
ebsd_mono_2NVa3 = dataset_rotation(ebsd_mono_2NVa3,[-5,-5,0],'axis','keep','keepXY');
ebsd_mono_38NVa1 = dataset_rotation(ebsd_mono_38NVa1,[10,-5,0],'axis','keep','keepXY');
...
ebsd_met_LOWDR = dataset_rotation(ebsd_met_LOWDR,[0,25,0],'axis','keep','keepXY');
ebsd_met_MEDDR = dataset_rotation(ebsd_met_MEDDR,[0,30,0],'axis','keep','keepXY');
ebsd_met_HIGH = dataset_rotation(ebsd_met_HIGH,[0,0,30],'axis','keep','keepXY');
ebsd_met_1TDa1 = dataset_rotation(ebsd_met_1TDa1,[-5,5,0],'axis','keep','keepXY');
ebsd_met_2NVa3 = dataset_rotation(ebsd_met_2NVa3,[-5,-5,0],'axis','keep','keepXY');
ebsd_met_38NVa1 = dataset_rotation(ebsd_met_38NVa1,[10,-5,0],'axis','keep','keepXY');
...
for sgi = 1:length(mono_ebsd_list)
  ...
  if strcmp(name_list{sgi},'AC 350') == 1
    grains_mono = create_grains(mono_ebsd_list{1,sgi},'misorientation',misorientation,'smallest_grain',small_grain/9,'smoothing',1,'fill_gaps','no','phase_name','Monoclinic ZrO$$_2$$')
    grains_met = create_grains(met_ebsd_list{1,sgi},'misorientation',misorientation,'smallest_grain',small_grain/9,'smoothing',3,'fill_gaps','no','phase_name','HCP Zr')
  elseif strcmp(name_list{sgi},'MIBL Ex High DR') == 1
    grains_mono = create_grains(mono_ebsd_list{1,sgi},'misorientation',misorientation,'smallest_grain',small_grain/9,'smoothing',1,'fill_gaps','no','phase_name','Monoclinic ZrO$$_2$$')
    grains_met = create_grains(met_ebsd_list{1,sgi},'misorientation',misorientation,'smallest_grain',small_grain/9,'smoothing',3,'fill_gaps','no','phase_name','HCP Zr') 
  else    
...
for sgi = 1:length(mono_ebsd_list)
  ...
  %plot_pf(odf_data,desired_pole_figures,'crys_sym',mono_ebsd_list{1,sgi}('Monoclinic ZrO$$_2$$').CS,'titles',name_list{1,sgi},'cbar_limit',70)
  ...
  %plot_pf(odf_data,desired_pole_figures,'crys_sym',met_ebsd_list{1,sgi}('HCP Zr').CS,'titles',name_list{1,sgi},'cbar_limit',320)
  ...
  grain_dimension_hist_caliper(grainsets_mono{1,sgi}('Monoclinic ZrO$$_2$$'),'bin_size',10,'max_size',1200,'units','nm','max_percentage',7)
  ...
  %orientation_deviation_histogram_osc(mono_ebsd_list{1,sgi},'bin_size',3,'max_y',35,'titles',name_list{1,sgi},'phase_name','Monoclinic ZrO$$_2$$','ref_text_comp',[1,0,-3])
end
