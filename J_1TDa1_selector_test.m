...
data_full = [pname '1TDa1_r1.6_ee_0.4_more_phases_Fuzz.ctf'];
...
ebsd_full = x_section_correction(ebsd_full,'EBSD','scan_rotation',180)
...
odf = make_ODF(ebsd_full(phase_of_interest))
odf_data= calcODF(ebsd_full(phase_of_interest).orientations,'halfwidth', 3*degree)
desired_pole_figures = [[0,0,0,2,"plane"];[1,1,-2,0,"plane"]];
plot_pf(ebsd_full,desired_pole_figures,'crys_sym',ebsd_full(phase_of_interest).CS)
plot_pf(odf_data,desired_pole_figures,'crys_sym',ebsd_full(phase_of_interest).CS)
