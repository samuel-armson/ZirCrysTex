function gr = create_grains(data_in,desired_pfs,varargin)
	...
	if strcmp(p.Results.fill_gaps,'no') == 1
		[grains_dirty,ebsd_full(phase_of_interest).grainId] = calcGrains(ebsd_full(phase_of_interest),'angle',Grain_mis_param,'boundary','tight');
	else
	...
	disp('Removing small grains...')
  	...
  	%[grains_clean,ebsd_full(phase_of_interest).grainId] = calcGrains(ebsd_full(phase_of_interest),'angle',Grain_mis_param,'boundary','tight');
