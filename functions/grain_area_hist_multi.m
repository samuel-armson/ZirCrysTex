  ...
  for grainset = 1:length(data_in)
    ...
    if strcmp(p.Results.plot_type,'hist_line') == 1
      ...
      elseif strcmp(p.Results.plot_type,'scatter') == 1
        if strcmp(p.Results.legend_labels,'none') == 1
          scatter(x_vals,y_vals,'DisplayName',num2str(grainset),'MarkerFaceColor',cmap(row_number, :),'MarkerType','_')
        else
          scatter(x_vals,y_vals,'DisplayName',num2str(p.Results.legend_labels(grainset)),'MarkerFaceColor',cmap(row_number, :),'MarkerType','_')
        end
    end	 
